...
def ajustar_brilho_contraste(imagem, brilho=8, contraste=1.10):
...
def aplicar_suavizacao(imagem, metodo="Gaussiano"):
    ...
    if metodo == "Bilateral":
        return cv2.bilateralFilter(imagem, 7, 45, 45)
...
def gerar_pdf_da_imagem(imagem):
    buffer = BytesIO()
    Image.fromarray(imagem).convert("RGB").save(buffer, format="PDF", resolution=100.0)
    return buffer.getvalue()
...
def gerar_zip_com_imagens(etapas):
    ...
    with zipfile.ZipFile(buffer_zip, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        zip_file.writestr("resultado_final.pdf", gerar_pdf_da_imagem(etapas["10_resultado_final"]))
    buffer_zip.seek(0)
...
def validar_quadrilatero(pontos, area_imagem):
    ...
    return all(45 <= angulo <= 135 for angulo in angulos)
...
def recorte_fino_documento(imagem):
    ...
    _, mascara_texto = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
def centralizar_em_canvas(imagem, margem=30, cor_fundo=255):
    ...
    if len(imagem.shape) == 2:
        ...
        canvas = np.full((altura + margem * 2, largura + margem * 2), cor_fundo, dtype=np.uint8)
        canvas[margem:margem + altura, margem:margem + largura] = imagem
        return canvas
    altura, largura = imagem.shape[:2]
    canvas = np.full((altura + margem * 2, largura + margem * 2, 3), cor_fundo, dtype=np.uint8)
    canvas[margem:margem + altura, margem:margem + largura] = imagem
    return canvas
...
def detectar_por_contorno_quadrilateral(imagem):
    ...
    bordas = cv2.Canny(suavizada, st.session_state.canny_limiar1, st.session_state.canny_limiar2)
    kernel = np.ones((5, 5), np.uint8)
    bordas_unidas = cv2.dilate(bordas, kernel, iterations=1)
    bordas_unidas = cv2.morphologyEx(bordas_unidas, cv2.MORPH_CLOSE, kernel)
    contornos, _ = cv2.findContours(bordas_unidas, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contorno in contornos[:20]:
        ...
        for fator in [0.015, 0.02, 0.03, 0.04]:
            ...
            cv2.drawContours(desenho, [pontos_originais.astype(np.int32).reshape(-1, 1, 2)], -1, (0, 255, 0), 8)
...
def detectar_por_regiao_clara(imagem):
    ...
    if not contornos:
        return {"sucesso": False, "documento": imagem, "contorno_desenhado": imagem.copy(), "metodo": "Fallback falhou"}
    contornos = sorted(contornos, key=cv2.contourArea, reverse=True)
    area_total = imagem.shape[0] * imagem.shape[1]
    desenho = imagem.copy()
    for contorno in contornos[:10]:
        ...
        return {"sucesso": True, "documento": recorte, "contorno_desenhado": desenho, "metodo": "Região clara"}
    return {"sucesso": False, "documento": imagem, "contorno_desenhado": desenho, "metodo": "Fallback falhou"}
...
def gerar_cinza_aprimorado(documento):
    ...
    clahe = cv2.createCLAHE(clipLimit=float(st.session_state.clahe_clip), tileGridSize=(8, 8))
    ...
    return aplicar_suavizacao(ajustada, metodo=st.session_state.metodo_suavizacao)
...
def gerar_scanner_pb(imagem_cinza):
    ...
    return cv2.morphologyEx(adaptativa, cv2.MORPH_OPEN, kernel)
...
def escolher_resultado_final(documento, cinza_aprimorado, scanner_pb):
    ...
    documento_colorido = saturacao_media > 25
    pb_agressivo_demais = proporcao_preto > 0.35
    pb_claro_demais = proporcao_preto < 0.015

    if documento_colorido or pb_agressivo_demais or pb_claro_demais:
        return cinza_aprimorado
...
st.sidebar.slider("Brilho", min_value=-100, max_value=100, key="brilho")
st.sidebar.slider("Contraste", min_value=0.5, max_value=3.0, step=0.05, key="contraste")
st.sidebar.slider("Canny - Limiar inferior", min_value=0, max_value=255, key="canny_limiar1")
st.sidebar.slider("Canny - Limiar superior", min_value=0, max_value=255, key="canny_limiar2")
st.sidebar.slider("CLAHE - Clip limit", min_value=1.0, max_value=5.0, step=0.1, key="clahe_clip")
st.sidebar.slider("Bloco da limiarização adaptativa", min_value=3, max_value=99, step=2, key="bloco")
st.sidebar.slider("Constante da limiarização adaptativa", min_value=0, max_value=30, key="constante")
st.sidebar.slider("Margem do canvas", min_value=0, max_value=100, key="margem_canvas")

arquivo = st.file_uploader("Carregue uma imagem de documento", type=["jpg", "jpeg", "png", "bmp", "webp"])
